...
Reads the SkillsFuture Courses dataset (downloaded by
:class:`DataDownloader` as either a converted CSV or the original xlsx),
optionally filters by industry keywords, and applies user-selected
filters and sorting.

Performance note: the full parsed course list is cached in memory via
:mod:`app.services._cache` and only re-built when the underlying source
file changes on disk. Per-request work therefore reduces to a cheap
in-memory filter rather than a full file parse.
"""
...
# Per-industry keyword matchers are memoised here: the keyword lists are
# static, so each industry's Aho-Corasick automaton is built at most once for
# the lifetime of the process. Matchers are case-sensitive because callers
# pass already-lowercased text (preserving exact str.lower() semantics).
_INDUSTRY_MATCHERS: dict = {}


def _get_industry_matcher(industry: str, keywords):
    """Return a cached keyword matcher for *industry*, building it on first use."""
    key = industry.lower().strip()
    matcher = _INDUSTRY_MATCHERS.get(key)
    if matcher is None:
        matcher = _fast.make_matcher(keywords, case_insensitive=False)
        _INDUSTRY_MATCHERS[key] = matcher
    return matcher
...
class CourseService:
    ...
    def get_courses_data(
        ...
        """Load courses from the SkillsFuture dataset with optional filtering.

        The full parsed course list is cached in memory; this method only
        applies industry-keyword filtering and the user-selected sort /
        filter parameters, both of which are O(n) over an already-parsed
        list and complete in milliseconds.
        ...
        """
        success, source_path = self.downloader.download_skillsfuture_courses_csv()

        if not success:
            return False, [], "Error retrieving courses"

        try:
            all_courses = PARSED_FILE_CACHE.get_or_load(
                source_path, self._parse_source_file, extra_key="courses"
            )
            ...
            if industry:
                ...
                if industry_kws:
                    ...
                    # The corpus of (lowercased) titles is marshalled into the
                    # native matcher once and cached, keyed by the source
                    # file's mtime — so per request only the small keyword set
                    # is involved and the 25k-title scan stays entirely in C++.
                    title_index = PARSED_FILE_CACHE.get_or_load(
                        source_path,
                        lambda _p, c=all_courses: _fast.make_title_index(
                            [course.title.lower() for course in c]
                        ),
                        extra_key="courses_title_index",
                    )
                    matcher = _get_industry_matcher(industry, industry_kws)
                    matched_idx = title_index.filter(matcher, 150)
                    courses = [all_courses[i] for i in matched_idx]
                else:
                    courses = all_courses[:150]
            else:
                courses = all_courses[:150]

            print(
                f"Cached pool of {len(all_courses)} courses, "
                f"returning {len(courses)} after industry filter"
            )
    ...
    # ------------------------------------------------------------------
    # Source-file parsing (run once per refresh, cached thereafter)
    # ------------------------------------------------------------------

    def _parse_source_file(self, path: str) -> List[Course]:
        """Parse the SkillsFuture dataset into a list of Course objects.

        Dispatches to the CSV or xlsx reader based on file extension. This
        is the slow path — invoked once per cache miss, then memoised in
        :data:`PARSED_FILE_CACHE` until the underlying file's mtime changes.

        Args:
            path: Path to either ``SkillsFutureCourses.csv`` (preferred,
                produced by :class:`DataDownloader`) or the original xlsx.

        Returns:
            list[Course]: All parsed courses, up to a 25 000-row scan cap.
        """
        if path.lower().endswith('.csv'):
            return self._parse_csv(path)
        return self._parse_xlsx(path)

    def _parse_csv(self, csv_path: str) -> List[Course]:
        """Parse the converted CSV produced by :class:`DataDownloader`.

        Uses the stdlib :mod:`csv` module, which is dramatically faster
        than openpyxl — typically 10-50× on the same input, especially on
        Windows.
        """
        courses: List[Course] = []
    ...
    def _parse_xlsx(self, xlsx_path: str) -> List[Course]:
        """Fallback xlsx parser used when CSV conversion is unavailable."""
        courses: List[Course] = []
